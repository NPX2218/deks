...
class MenuBarManager: NSObject {
    ...
    func updateTitle() {
        ...
        let activeWs = WorkspaceManager.shared.workspaces.first { $0.id == WorkspaceManager.shared.activeWorkspaceId }
...
class MenuBarViewController: NSViewController {
    ...
    func reload() {
        ...
        let activeName = (focused?.appName == "Deks" || focused == nil) ? "No active window" : focused!.appName
        ...
        for (index, ws) in workspaces.enumerated() {
            ...
            for win in ws.assignedWindows {
                let name: String
                if let mapped = NSWorkspace.shared.runningApplications.first(where: { $0.bundleIdentifier == win.bundleID })?.localizedName {
                    name = mapped
                } else {
            ...
            let primaryStr = NSMutableAttributedString(string: " \(ws.name)", attributes: [.font: NSFont.boldSystemFont(ofSize: 14), .paragraphStyle: pstyle])
            if !shortcutHint.isEmpty {
                let hintStr = NSAttributedString(string: shortcutHint, attributes: [.font: NSFont.systemFont(ofSize: 11, weight: .semibold), .foregroundColor: NSColor.tertiaryLabelColor])
                primaryStr.append(hintStr)
            }
            primaryStr.append(NSAttributedString(string: "\n", attributes: [.font: NSFont.boldSystemFont(ofSize: 14)]))
            
            let attrStr = NSMutableAttributedString()
            attrStr.append(primaryStr)
            attrStr.append(NSAttributedString(string: "  " + subtitle, attributes: [.font: NSFont.systemFont(ofSize: 11), .foregroundColor: NSColor.secondaryLabelColor]))
        ...
        let newWsBtn = NSButton(title: "+ New workspace", target: self, action: #selector(newWorkspaceClicked))
        newWsBtn.isBordered = false
        newWsBtn.contentTintColor = .labelColor
        stackView.addArrangedSubview(newWsBtn)
        
        let settingsBtn = NSButton(title: "Settings...", target: self, action: #selector(settingsClicked))
        ...
        if #available(macOS 13.0, *) {
            ...
            let loginBtn = NSButton(title: status ? "Launch at Login (Activated)" : "Enable Launch at Login", target: self, action: #selector(toggleLogin))
    ...
    @objc private func quickAssignChanged(_ sender: NSPopUpButton) {
        ...
        if let idx = WorkspaceManager.shared.workspaces.firstIndex(where: { $0.id == targetId }) {
            let ref = WindowRef(id: focused.id, bundleID: focused.bundleID, windowTitle: focused.currentTitle, matchRule: .exactTitle(focused.currentTitle))
    ...
    @objc private func newWorkspaceClicked() {
        ...
        let ws = WorkspaceManager.shared.createWorkspace(name: "Workspace \(count + 1)", color: .purple)
...
// Helper association
@MainActor fileprivate var associatedIdKey: UInt8 = 0
extension NSButton {
    var associatedId: UUID? {
        get { objc_getAssociatedObject(self, &associatedIdKey) as? UUID }
        set { objc_setAssociatedObject(self, &associatedIdKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
}
